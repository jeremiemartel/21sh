...
/*   Updated: 2019/03/28 18:06:34 by ldedier          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			if (ptr->next != NULL) //(1) a) aBb with b = ptr->next
			{
				...
				*/
				...
				if (ref == &cfg->symbols[C])
				{
					...
					ft_printf("\n");
					ft_printf("beta first sets:\n");
					sh_process_print_set(cfg, first_sets);
					ft_printf("\n");
					sh_print_symbol(ref);
					ft_printf(" follow sets:\n");
...
int		sh_process_follow_sets_symbol(t_cfg *cfg, t_symbol *symbol)
{
	...
	while (j < NB_NOTERMS)
	{
		if (cfg->symbols[i].follow_sets[END_OF_INPUT]
				&& sh_process_follow_sets_symbol_ref(cfg,
					&cfg->symbols[i], symbol))
			changes = 1;
		i++;
		j++;
	}
