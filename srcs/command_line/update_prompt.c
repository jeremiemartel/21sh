...
/*   Updated: 2019/05/23 18:33:05 by ldedier          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "sh_21.h"

int		update_prompt_cwd(char **new_prompt)
{
	char	*cwd;
	char	*path;
...
int		update_prompt_context(t_command_line *command_line, char **new_prompt)
{
	if (command_line->context == E_CONTEXT_STANDARD)
	{
		if (update_prompt_cwd(new_prompt))
			return (FAILURE);
...
int		update_prompt(t_command_line *command_line)
{
	...
	if (update_prompt_context(command_line, &new_prompt))
		return (FAILURE);
...
int		update_prompt_from_quote(t_command_line *command_line, char quote)
{
	...
	return (update_prompt(command_line));
}
